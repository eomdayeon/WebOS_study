...
service.register("power", function(message) {
    console.log(logHeader, "SERVICE_METHOD_CALLED:/power");
    console.log("power callback");
    var subscribe = message.payload.subscribe;

    service.call("luna://com.webos.service.power2/getState", {"subscribe":subscribe}, function(m2) {
        console.log(logHeader, "SERVICE_METHOD_CALLED:luna://com.webos.service.power2/getState");
        const response =  m2.payload.state;
        const response2 = m2.payload.subscribed;

        console.log(response);

        message.respond({
            returnValue :true,
            Response: "Current state : " + response + ", subscribed : " + response2
        });
    });
});
...
service.register("listfiles",function(message){
    ...
    fs.readdir(path,function(err,files){
        if(err){
            message.respond({
                returnValue: false,
                errorCode:"Error when reading files"
            });
        }
        else{
            message.respond({
